...
peptide_nmds <- nmdsplots ("peptide", "Protein")  
fc_nmds <- nmdsplots ("FC", "Flow Cytometry")
pigment_nmds <- nmdsplots ("pigm", "Pigment")
...
#this will be the ende all be all cluster annotations. 
...
DE sliding plot
```{r}
cluster_order <- c('Fructose bisphosphatase', 
                   'ISIP-1',
                   'ISIP-2A', 
                   'ISIP-3',
                   'Fasciclin',
...
DE_slide <- function (effect, significance, title) {
  ggplot (BA_DE3, aes(x= {{effect}}, 
                   y = reorder (cluster_annotation, {{effect}}),
                   color = taxon_F, 
                   #size = (BA1_grpnorm),
                   shape = {{significance}}))+ 
...
A <- DE_slide (BA1_Fe_logFC, BA1_ironDE, "BA1 Fe Effect" ) +theme (legend.position = c(0.01, 0.97), legend.justification = c(0,0.95), legend.key.size = unit (0.4, "cm")) + guides(colour = guide_legend(override.aes = list(size=3))) + theme(legend.margin=margin(t=-0.7,  r=0, b=0, l=0, unit="cm")) 


B <- ggplot (BA_DE3, aes(x = taxon_F, 
...
C <- DE_slide (BA2_Fe_logFC, BA2_ironDE, "BA2 Fe Effect" ) +theme (legend.position = "none")
  
D <- ggplot (BA_DE3, aes(x = taxon_F, 
...
plot_grid (x, y, nrow = 2, labels = c("A", "B"))
