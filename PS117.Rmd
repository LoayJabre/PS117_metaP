...
maplegendvert <- basemap(limits = c(-55),
        bathymetry = TRUE, #this adds the bathymetry (depth info) 
        glaciers = FALSE)+ 
        labs(x = "", y = "") +
        theme(legend.title = element_text(size = 10, face = "bold"),
...
maplegendvert <- get_legend(maplegendvert)
maplegendvert <- as_ggplot(maplegendvert)
maplegendvert
...
inset <- ggplot(filter (nonproteindata_long, !grepl ("0.35", value) & grepl("DFe56|DMn", variable) & grepl ("T0", full_treatment_name)), 
       ...
       labs(x = expression (""), y = expression ("Initial concentration (nM)"))+
       facet_wrap(~bioassay)+
       theme_bw()+
...
sino3drawdown <- ggplot(filter (si_no3_drawdown, grepl ("8|0", daycount) & grepl("BA1|", bioassay)), 
  ...
  stat_summary(fun.data =  mean_sd, geom = "errorbar", show.legend = F, size = 1.3, width = 0.1) +
  scale_color_manual(name="Temperature",
                     breaks = c("LT","HT"),
                     labels = c("LT", "HT"),
                     values = c('black', 'darkorange2','black')) +
...
png("cbiomes1.png", width= 12, height=6, units="in", res=900) 

ggdraw(plot_grid(poc, NULL, no3,
          chla, NULL,  po4, ncol = 3, rel_widths = c(1,0.1,1), align = "vh")) + 
          draw_plot (vertical_legend , x = 0.81, y= 0.13, width = 0.05, height = 0.05) 
dev.off()
...
#png("ps117_figure1_highres_20230519.pdf", width=19.2, height=9.4, units="in", res=900) 
...
Figure 1 plot 2023-04-20- long version, looks better for the paper
```{r}
x <- plot_grid(NULL, NULL, inset, 
               fvfm, NULL, no3,
               chla, NULL, po4,
               poc, NULL, sino3drawdown,
...
#png("ps117_figure1_20230606.png", width= 13, height=12, units="in", res=900) 
#pdf("ps117_figure1_highres_20230523.pdf", width= 13, height=12) 
...
#write.csv(allgroups_aggregated, "proteingroups.csv")
...
pennategroup <- protein_taxgroups("Pennate", "Pennate diatom protein fraction") 
centricgroup <- protein_taxgroups("Centric", "Centric diatom protein fraction")  
phaeogroup <- protein_taxgroups("Phaeocystis", "Phaeocystis protein fraction") 



png("cbiomes2.png", width= 13, height=3, units="in", res=900) 
plot_grid (pennategroup, centricgroup, phaeogroup, nrow = 1)
...
For Erin's cbiomes talk - delete / not really useful 
```{r}
compartment_aggregated_group <- filter (compartment_aggregated_group, grepl ("BA1", bioassay))
...
ggplot(filter (compartment_aggregated_group, grepl (compartments, compartment) & grepl (taxon, taxon_F)),
  ...
  theme_bw()+
  theme(axis.text=element_text(face = "bold", size = 16, color = "black"),
...
#double check - separate plots for each of the groups for BA1 



#phaeocystis
phaeochloro <- grpnorm_compartment ("^chloro$", "Centric",  atop ("Taxon normalized", paste ("Photosynthetic protein (%)")))+
  ylim(0, 33) + 
  ggtitle("Centric diatoms - Bioassay 1") + theme (plot.title = element_text(size = 14))


phaeorubisco <- grpnorm_compartment ("^rubisco$", "Centric", atop ("Taxon normalized", paste ("RuBisCO protein (%)"))) +
  ylim(0,NA)
    
phaeoribosome <- grpnorm_compartment ("^ribosome$", "Centric", atop ("Taxon normalized", paste ("Ribosomal protein (%)"))) +
  ylim(0,23) 
 
a <- plot_grid (phaeochloro, phaeorubisco, phaeoribosome ,
               nrow = 3, 
            labels = c("A", "B", "C"))


png("ps117_centricphotoriborubisco_BA1_20230529.png", width=7, height=10, units="in", res=900) 

ggdraw (a) + draw_plot (vertical_legend, x = 0.8, y = 0.01, width = 0.25, height = 0.18)  
...
#Figure 4 - Sample-normalized major protein groups and N:P 2023-05-26 
```{r}
...
#png("ps117_ps117_samplenorm_majorproteins_20230526.png", width=13, height=7, units="in", res=900) 
...
T0_T8_FenoFe((POC_uM)/(POP_nM), POC:PON~~(M:M))
...
#playing arund can delete after -- 
T0_T8_FenoFe(((POC_uM)/POP_nM), POC:POM~~(M:M))# + 
                 #expand_limits(y = 0) +
                 scale_x_discrete(limits=c("T0","noFe","Fe"), 
                                  labels = c("T0", "-Fe", "+Fe"))+
                 theme(axis.title.y=element_text(size=14, color = "black")) + 
     geom_hline (aes(yintercept = 6.6), col = "black", lty = 2, lwd=0.6 )
```
...
#png("ps117_figure4_20230604.png", width=11, height=7.5, units="in", res=900) 


ggdraw (a) + draw_plot (vertical_legend, x = 0.825, y = 0.553, width = 0.25, height = 0.18)  
...
sample-normalized major protein groups figure 
```{r}
a <- plot_grid (photosyn, NULL, particulatenp, 
           rubisco, NULL, npdrawdwon,
           ribosome,  NULL, metaanaNP,
           rel_widths = c(1,0.04,1), 
           ncol = 3, 
           align = "vh", axis ="bt", 
           labels = c ("A", "", "D", 
...
#png("ps117_photoriborubisco_NP_20230527.png", width=12, height=11, units="in", res=900) 
...
#Figure 5,6 - metals and DE 2023-05-07

Metal Plots - use only PON , and the rest in supplement
```{r}
...
Fe57PON <- metalplot ((PFe57_nM)/(PON_uM),P ^{57}*"Fe":PON~~(mM:M)) 
...
BA_DE$cluster_annotation <- ifelse (BA_DE$cluster ==  "clust_1814", "ISIP-1", 
                            ...
                            ifelse (BA_DE$cluster ==  "clust_6573", "Cd-CA", 
                            ...
                            ifelse (BA_DE$cluster ==  "clust_2460", "Ferredoxin", 
...
FeDE_BA1 <-   DE_slide (BA1_Fe_logFC, BA1_ironDE, "BA1 Fe-addition effect" ) + 
...
FeDE_BA2 <-  DE_slide (BA2_Fe_logFC, BA2_ironDE, "BA2 Fe-addition effect" ) + 
...
cdcacalc <-  ggplot(filter (alldata2, grepl ("T0|T8", full_treatment_name) & grepl ("BA2|", bioassay)), 
       aes( x = bioassay, 
...
Figure 5 _ 20230611 (i botched these togethe quickly so I can have a figure)
```{r}
...
png("ps117_figure5_20230616.png", width=16, height=10, units="in", res=900)
...
#fract_diatoms
#fract_haptophytes
BA1 <- filter (nonproteindata, grepl ("BA1", bioassay) & grepl ("8", daycount))
x <- aov (as.numeric(PCd_nM/PON_uM)~as.factor(iron_treatment)*as.factor(temperature_treatment), data = BA1)
TukeyHSD(x)
...
compartment$taxon <- ifelse (compartment$F ==  "Polar-centric-Mediophyceae", "centric_diatom", 
                     ifelse (compartment$F ==  "Radial-centric-basal-Coscinodiscophyceae", "centric_diatom", 
                     ifelse (compartment$F ==  "Raphid-pennate", "pennate_diatom", 
                     ifelse (compartment$F ==  "Araphid-pennate", "pennate_diatom", 
                     ifelse (compartment$F ==  "Phaeocystaceae", "phaeocystis",
                     
                     ifelse (compartment$class_x ==  "Bacillariophyta_X", "ambig_diatom", 
...
#write.csv(compartment_aggregated_group2, "ps117_circlepack_data_20230526.csv", row.names = F)
```
...
edges <- read.csv ("ps117_circlepack_edges_20230526.csv")

vertices_BA1 <- read.csv ("ps117_circlepack_B1T0vertices_20230526.csv")
vertices_BA2 <- read.csv ("ps117_circlepack_B2T0vertices_20230526.csv")


mygraph_BA1 <- graph_from_data_frame(edges, vertices=vertices_BA1 )
mygraph_BA2 <- graph_from_data_frame(edges, vertices=vertices_BA2 )


set.seed(10)
a <- ggraph(mygraph_BA1, layout = 'circlepack', weight = size) +  #weight = size adjusts the circle size
  ...
  scale_fill_manual(values=c("allproteins" = "NA",
         ...
         scale_color_manual(values=c("empty" = "NA",
                              "notempty" = "black")) +
annotate ("text", x = -0.4, y = 0.8, label = "Bioassay 1", size = 8) +                            
         geom_node_text(aes(label = proteinlabel), size = 3) +
         geom_node_label(aes(label = taxonlabel), label.size = 0, fontface = "bold",
                         
            
                               size = ifelse(vertices_BA1$taxonlabel == "Unknown", 1.8,
                               ifelse(vertices_BA1$taxonlabel == "Ambiguous domain", 2.3,
                               ifelse(vertices_BA1$taxonlabel == "Dinophytes", 2.5,
                               ifelse(vertices_BA1$taxonlabel == "Other eukaryotes", 2.3,
                               ifelse(vertices_BA1$taxonlabel == "Other hapto.", 3,
                               ifelse(vertices_BA1$taxonlabel == "Bacteria", 2,
                               ifelse(vertices_BA1$taxonlabel == "Ciliates", 3,
                                      3.5))))))),
                             
                        fill = ifelse(vertices_BA1$taxonlabel == "empty", "NA", "white"), 
                        color = ifelse(vertices_BA1$taxonlabel == "empty", "NA", "black"), 
                                 nudge_y = ifelse (vertices_BA1$taxonlabel == "Ambiguous eukaryotes", 0.1, #BA1
                                 ifelse (vertices_BA1$taxonlabel == "Unknown", -0.08, 0))) +
...
set.seed(10)
b <- ggraph(mygraph_BA2, layout = 'circlepack', weight = size) +  #weight = size adjusts the circle size
      geom_node_circle(aes(fill = as.factor(colors), 
                         color = as.factor (circles),
                         group = depth))+
  #geom_node_circle()+
  scale_fill_manual(values=c("allproteins" = "NA",
         ...
         scale_color_manual(values=c("empty" = "NA",
                              "notempty" = "black")) +
annotate ("text", x = -0.4, y = 0.8, label = "Bioassay 2", size = 8) +                            
  #filter = leaf     
    geom_node_text(aes(label = proteinlabel), size = 3)+
        
   geom_node_label(aes(label = taxonlabel), label.size = 0, fontface = "bold",
                               size = ifelse(vertices_BA2$taxonlabel == "Unknown", 2,
                               ifelse(vertices_BA2$taxonlabel == "Ambiguous domain", 2.3,
                               ifelse(vertices_BA2$taxonlabel == "Dinophytes", 2.5,
                               ifelse(vertices_BA2$taxonlabel == "Other eukaryotes", 2.3,
                               ifelse(vertices_BA2$taxonlabel == "Other hapto.", 3,
                               ifelse(vertices_BA2$taxonlabel == "Bacteria", 3,
                               ifelse(vertices_BA2$taxonlabel == "Ciliates", 3,
                                      3.5))))))),

                        fill = ifelse(vertices_BA2$taxonlabel == "empty", "NA", "white"), 
                        color = ifelse(vertices_BA2$taxonlabel == "empty", "NA", "black"), 
                        nudge_y = ifelse (vertices_BA2$taxonlabel == "Ambiguous eukaryotes", 0.1,
                                  ifelse (vertices_BA2$taxonlabel == "Centric diatoms", -0.04,
                                          0))) + 
...
#png("ps117_circlepack_20230617.png", width=8, height=12, units="in", res=900) 
...
# 
# ggraph(mygraph_BA2, layout = 'treemap', weight = size) +  #weight = size adjusts the circle size
#       geom_node_tile(aes(fill = as.factor(taxonlabel), 
#                          color = as.factor (circles),
#                          group = depth))+
# 
#          scale_color_manual(values=c("empty" = "NA",
#                               "notempty" = "black")) +
#   #filter = leaf     
#     geom_node_text(aes(label = proteinlabel, filter = leaf), size = 3)+
#         
#    #geom_node_label(aes(label = taxonlabel), label.size = 0, fontface = "bold")+
#                               
# 
