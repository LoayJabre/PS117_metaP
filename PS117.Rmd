...
pennategroup <- protein_taxgroups("Pennate", "Pennate diatoms") + theme(axis.text.x = element_blank()) + ggtitle("Fraction of total protein")
...
#png("ps117_figure2_highres_20230507.png", width=20, height=10, units="in", res=900) 
...
#photosynthesis
a <- grpnorm_compartment ("^chloro$", "Pennate di", "Pennate diatoms") +  # atop ("Pennate diatom", paste ("photosynthetic protein fraction"))) 
  ylim(0,0.33) + 
  theme (axis.text.x = element_blank())+
  ggtitle("Photosynthetic protein fraction")

b <- grpnorm_compartment ("^chloro$", "Centric di", "Centric diatoms") + 
  ylim(0,0.33) + 
  theme (axis.text.x = element_blank())

c <- grpnorm_compartment ("^chloro$", "Phaeocys", "Phaeocystis") +
  ylim(0,0.33)


#rubisco
d <- grpnorm_compartment ("^rubisco$", "Pennate di", "Pennate diatoms") +
  ylim(0,0.07) + 
  theme (axis.text.x = element_blank()) + 
  theme(axis.title.y = element_blank())+ 
  ggtitle("RuBisCo protein fraction")

e <- grpnorm_compartment ("^rubisco$", "Centric di", "Centric diatoms") + 
  ylim(0,0.35) + 
  theme (axis.text.x = element_blank()) + 
    theme(axis.title.y = element_blank())


f <- grpnorm_compartment ("^rubisco$", "Phaeocys", "Phaeocystis")  +
  ylim(0,NA) + 
    theme(axis.title.y = element_blank())

#ribosome
g <- grpnorm_compartment ("^ribosome$", "Pennate di", "Pennate diatoms") +
  ylim(0,0.225) + 
  theme (axis.text.x = element_blank())+
  ggtitle("Ribosomal protein fraction") + 
  theme(axis.title.y = element_blank())


h <- grpnorm_compartment ("^ribosome$", "Centric di", "Centric diatoms") +
  ylim(0,0.225) + 
  theme (axis.text.x = element_blank()) + 
  theme(axis.title.y = element_blank())

i <- grpnorm_compartment ("^ribosome$", "Phaeocys", "Phaeocystis") +
  ylim(0,0.225) + 
  theme(axis.title.y = element_blank())
```
...
B <- plot_grid (npdrawdwon, particulatenp, 
                labels = c("J", "K"),
                ncol = 1,
                rel_heights = c(1,1),
                align = "v")
...
#png("ps117_figure3_highres_20230507.png", width=17, height=8, units="in", res=900) 

plot_grid (A, NULL, B, vertical_legend, ncol =4 , 
           rel_widths = c(3,0.05, 1, 0.22), align = "v", axis = "bt", 
