Load necessary packages another quick test
```{r}
library(cowplot)
library(data.table)
library(dplyr)
...
treatment_orders <- c("T0", "LT_noFe", "LT_Fe", "HT_noFe", "HT_Fe")
treatment_order <- c('T0_T0', 'LT_noFe', 'LT_Fe', 'HT_noFe', 'HT_Fe')
irontreatment_order <- c('T0', 'noFe', 'Fe')
```
...
map <- basemap(limits = c(-130, 100, -90,-55),
  ...
  xlab("")+
      theme(
        ...
        geom_spatial_point(data = bioassays, aes(x = lon, y = lat), color = "red", size = 4) +
...
t0_3 <- filter (t0_2, grepl ("BA1", bioassay))
...
map, nutrients, chla, fv/fmphysical oceanography stuff
```{r}
...
pigment_means$taxon_group2 <-   ifelse (pigment_means$taxon_group ==  "diatoms", "Bacillariophyta", 
                   ...
                   ifelse (pigment_means$taxon_group ==  "prasinophytes", "Prasinophyceae", 
...
fig2_pigmentmeans <-ggplot(pigment_means, aes(x = factor(treatment2, level = treatment_orders),
 y = taxon_group2, 
  fill = fract)) +  
...
allgroups$class_a <-   ifelse (allgroups$class ==  "Bacillariophyta", "Bacillariophyta", 
                    ...
                    ifelse (allgroups$class ==  "Chlorophyceae", "Prasinophyceae",
                    ifelse (allgroups$class ==  "Chlorophyta", "Prasinophyceae",
                    ifelse (allgroups$class ==  "Pyramimonadales", "Prasinophyceae",
                    ifelse (allgroups$class ==  "Prasinophyceae", "Prasinophyceae",
...
fig2_proteintopmeans <- ggplot(allgroups_topmeans, aes(x = factor(treatment2, level = treatment_orders), y = class, fill= norm_abundance))+ 
  geom_tile(color = "black", lwd = 0.7) +
...
fig2_proteinbottommeans <- ggplot(allgroups_bottommeans, aes(x = factor(treatment2, level = treatment_orders), y = class, fill= norm_abundance))+ 
  geom_tile(color = "black", lwd = 0.7) +
  scale_fill_gradientn(colours = c("deepskyblue2","white","darkred"),
...
pigment nmds and legend and a quick test here
```{r}
...
fig2_pigment_nmds <- ggplot(data.scores, aes(x = NMDS1, 
  ...
  theme (legend.text = element_text(size = 6, face ="bold", colour ="black"), 
    legend.position = "none", 
    #legend.margin=margin(r = ,unit ="cm"),
...
#png("diatom_genus.png", width=40*0.75, height=30*0.75, units="cm", res=600) 
...
#legend.justification = c(1,1), #this locks the legend in position so it doesnt move around as you zoom in and out of the plot

fig2_diatomspecies <- ggplot ((filter(diatoms_aggregated, grepl("Chae|Pseu", genus))), aes(
  ...
  facet_grid(.~ bioassay )+
  theme_bw()+
  ylab (expression(paste("Fraction contribution to \n total diatom proteins")))+
  #ylab ("Fraction contribution to total diatom proteins")+

  xlab ("") +
  scale_y_log10()+
  scale_shape_manual(name="",
                     values = c("Chaetoceros" = 16, "Pseudo-nitzschia" = 15))+ 
...
legend <- ggplot(data.scores, aes(x = NMDS1,
  ...
  scale_alpha_manual(breaks = c("Fe", "noFe", "T0") ,
                    values = c(1,0.2,0.2),
                    name = "Iron") +
  scale_fill_manual(values = c("HT" = "darkorange", "LT" = "black", "T0" = "blue"), guide = FALSE) +
  ...
  guides(alpha = guide_legend(override.aes = list(size=4.5)))
...
col3 <- plot_grid (fig2_diatomspecies, fig2_Sidrawdown, 
                   ncol = 1, 
                   align = "v", axis = "l", 
                  rel_heights =  c(1,0.8),
                   labels = c("D","E"))
...
```{r}
alldata$fulltreatment <- paste (alldata$bioassay, alldata$iron_treatment, sep = "_")
```



```{r}
 ggplot((filter(alldata, grepl("0|8", daycount))) , aes(
   ...
   scale_color_manual(name= "Bioassay",
               ...
               values = c('blue', "black","black", "blue", "black","black")) +
   stat_summary(fun = mean, geom = "point", show.legend = T, size = 5, stroke = 1.5, alpha = 0.7)+
   stat_summary(fun.data =  mean_se, geom = "errorbar", show.legend = F,  size = 1, width = 0.1)+
...
PONPOP <- ggplot((filter(alldata, grepl("0|8", daycount))) , aes(
  ...
  scale_color_manual(
   ...
   stat_summary(fun = mean, geom = "point", show.legend = T, size = 4, stroke = 1.5, alpha = 0.5)+
   stat_summary(fun.data =  mean_se, geom = "errorbar", show.legend = F,  size = 1, width = 0.1)+
   scale_shape_manual(name = "Iron",
                       breaks = c("Fe","noFe", "T0"), #this removes the T0 from the legend
                       labels = c("Fe", "noFe", "T0"),
...
phaeo_ribosome <- taxon_compartment_temp("Phaeocyst", 'ribo', "Phaeocystis \n ribosomal protein fraction") + xlab("")
phaeo_chloro <- taxon_compartment_temp("Phaeocyst", 'chloro', "Phaeocystis \n chloroplast protein fraction") + scale_y_continuous(labels = function(x) format(x, nsmall = 2))

pennate_ribosome <- taxon_compartment_temp("pennate|centric|Bacill", 'ribo', "Pennate diatom \n ribosomal protein fraction") +xlab("")
...
fullplot <- plot_grid(PONPOP, NULL, ribosomes, NULL,   phaeo_ribosome, NULL, pennate_ribosome, NULL, centric_ribosome,
          ...
          nrow = 2, 
          align = 'hv',
          rel_widths = c(1.4, 0.05, 1.4, 0.1,1,0.05,1,0.05,1))
...
alldata$full_treatment_names <- paste (alldata$temperature_treatment, alldata$iron_treatment, sep = "_")

fig3_metals <- function(yaxis, yaxislabel) {
  ggplot((filter(alldata, grepl("8", daycount))) , aes(
  y = ({{yaxis}}/(1000*PON_uM)),
    #y = ({{yaxis}}/(POP_nM)),
  x = factor(full_treatment_names, level = treatment_order),
  color = temperature_treatment))+
  facet_wrap(~bioassay)+
  scale_color_manual(name="Temperature",
                     breaks = c("LT","HT"),
...
fig3_metals (PFe57_nM, PFe57:PON)
...
#metals_Ni <- fig3_metals (POP_nM, PNi:PON~(M:M))


#temperature legend 
fig3_metallegend <- ggplot((filter(alldata, grepl("8", daycount))) , aes(
...
legend3a <- get_legend(fig3_metallegend)
legend3a <- as_ggplot(legend3a)
```

figure 4 proteins (see chucnk with same name below without a function) 2022-08-30
```{r}
protein_clusters <- function (taxonomic_groups, plot_title, clusternames) {
...
clusters4_aggregated$clust_annotation <-   ifelse (clusters4_aggregated$cluster ==  "clust_343", "ISIP-2A", 
                   ifelse (clusters4_aggregated$cluster ==  "clust_411", "Nitrate reductase", 
                            ...
                            ifelse (clusters4_aggregated$cluster ==  "clust_6573", "Cd Carbonic Anhydrase", 
                            ...
                            ifelse (clusters4_aggregated$cluster ==  "clust_655", "CA",
...
ggplot((filter(clusters4_aggregated, grepl(clusternames, clust_annotation))), aes(x = factor(fulltreatment, level = treatment_orders),
  y = zscore_BAnew, 
  shape   = clust_annotation))+ 
  #color = temperature_treatment))+ 
  stat_summary(fun = mean, geom = "point", size = 4, stroke = 1, alpha = 0.5, position= position_dodge(width=0.3))+
  stat_summary(fun.data =  mean_se, geom = "errorbar", show.legend = F,  size = 1.3, width = 0.1, position= position_dodge(width=0.3))+
  theme_bw() +
...
Mnproteins_diatoms <- protein_clusters ("centric|pennate|Bacillariop", "", "Fe-Mn|Mn-clus" )
...
#Figure 5
```{r}
plot_grid(NULL, metals_Cd, NULL, Cdproteins_diatoms, NULL, Cdproteins_phaeo, 
          nrow = 1,
          rel_widths = c(0.05, 1, 0.1, 1.3, 0.05, 1.3),
          align = 'h')
...
```{r}
#iron proteins
protein_clusters ("centric|pennate|Bacillariop", "", "Flavodox|Ferredox" )
protein_clusters ("Phaeo|Prymn", "", "Flavodox|Ferredox")
```
...
clusters4_aggregated_means$clust_annotation <-   ifelse (clusters4_aggregated_means$cluster ==  "clust_343", "ISIP-2A", 
                   ifelse (clusters4_aggregated_means$cluster ==  "clust_411", "Nitrate reductase", 
                            ...
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_6573", "Cd Carbonic Anhydrase", 
                            ...
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_655", "CA",
...
#DE data
```{r}
BA1 <- read.csv("ps117_BA1_DE_20220824.csv", header = T)
...
ggplot (DE, aes (x = abs(BA1_Fe_logFC), 
                              y = abs(BA2_Fe_logFC),
                           color = BA1_Fe_FDR <0.05 & BA1_Fe_FDR >0, 
                           shape = BA2_Fe_FDR < 0.05 & BA2_Fe_FDR > 0, ))+
  geom_point(alpha = 0.3, size =4, stroke =1)+
  geom_abline(slope = 1, color = "red", size= 0.5)+
scale_colour_manual(name = 'BA1 signficant', values = setNames(c('blue','black'),c(T, F)))+
scale_shape_manual(name = 'BA2 significant', values = setNames(c(16,1),c(T, F)))+
  ggtitle("Fe_DE, logFC comparison")+
...
x <- ggplot (DE, aes (x = BA2_temp_logFC, 
                              y = -log10(BA2_temp_FDR), 
                 color = F ))+
...
clusters4_aggregated$clust_annotation <-   ifelse ( clusters4_aggregated$cluster ==  "clust_1820", "Plastocyanin", 
                            ifelse (clusters4_aggregated$cluster ==  "clust_2934", "Multi-Cu Oxidase", 
                            ifelse (clusters4_aggregated$cluster ==  "clust_2501", "Fe-Mn SOD", 
                            ifelse (clusters4_aggregated$cluster ==  "clust_5562", "Cu-Zn SOD", 
                            ifelse (clusters4_aggregated$cluster ==  "clust_6573", "CDCA - centric", 
                            ifelse (clusters4_aggregated$cluster ==  "clust_655", "CDCA - pennate",
                            ifelse (clusters4_aggregated$cluster ==  "clust_1906", "Mn-cluster",
                            ifelse (clusters4_aggregated$cluster ==  "clust_812", "ZIP",
                            ifelse (clusters4_aggregated$cluster ==  "clust_225", "Cyt Oxidase",
                            ifelse (clusters4_aggregated$cluster ==  "clust_1217", "RNA polymerase",
                            ifelse (clusters4_aggregated$cluster ==  "clust_1372", "Urea transporter",
                            ifelse (clusters4_aggregated$cluster ==  "clust_320", "Urea Carboxylase",
                            clusters4_aggregated$cluster))))))))))))
...
clusters4_aggregated_means$clust_annotation <-   ifelse ( clusters4_aggregated_means$cluster ==  "clust_1820", "Plastocyanin", 
                            ...
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_6573", "CDCA - centric", 
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_655", "CDCA - pennate",
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_1906", "Mn-cluster",
                            ifelse (clusters4_aggregated_means$cluster ==  "clust_812", "ZIP",
