...
Loay Jabre
- This is a set of scripts that analyze and plot data for the PS117 paper. 
- This includes proteomics data and other measurements.

#I) Load packages - last updated: 2023-03-08
```{r}
#plotting 
library(ggplot2)
library(ggplotify)
...
#II) Plotting functions - last updated: 2023-03-09
```{r}
...
#III) Import data of interest and other useful functions - last updated: 2023-03-08
```{r}
...
#import protein data
proteindata_unnorm <- read.csv("all_ps117_taxon-function_peptides_non-normalized_injections-means_20230307.csv", header = T)
...
#Figure 1 last updated: 2023-03-08
- Map of Antarctica showing bioassay sites. last updated: 2023-03-08
```{r}
library(ggOceanMaps)
...
map <- basemap(limits = c(-130, 100, -90,-55),
        ...
        annotation_scale(location = "br", text_cex = 1.5, text_face = "bold") + #this is map scale
        geom_spatial_point(data = bioassay_latlon, aes(x = lon, y = lat), color = "red", alpha = 0.7, size = 6) +
        geom_spatial_text(size = 7, fontface = "bold", aes(label=bioassay_latlon$bioassay, x = bioassay_latlon$lon, 
...
dFeMn_inset <- ggplot(filter (nonproteindata_long, !grepl ("0.35", value) & grepl("DFe56|DMn", variable) & grepl ("T0", full_treatment_name)), 
       aes (x= variable,
       y = value))+
...
#map +  inset_element(dFeMn_inset, left = 0.65, bottom = 0.75, right = 1, top = 1, align_to = "full")

```

- Fv/Fm, Chla, POC, Nitrate, phosphate, silicate
```{r}
fvfm <- T0_T8_FenoFe (fvfm, F[v]/F[m]) + ylim(0, NA)
poc <- T0_T8_FenoFe (POC_uM, POC~mu*M)
chla <- T0_T8_FenoFe (total_chla_ug_L, Chl-a~~mu*g~L^-1)
no3 <- T0_T8_FenoFe (NO3_uM, NO[3]~mu*M) + ylim(5, 30)
po4 <- T0_T8_FenoFe (PO4_uM, PO[4]~mu*M) + ylim(0, 2) 
...
legend
```{r}
fig1_legend <-  ggplot(filter (nonproteindata, grepl ("T0|T8", full_treatment_name) ), 
       aes(x=iron_treatment, 
       y=fvfm,
...
plot_grid(map, NULL, x, fig1_legend,
          ncol = 4, 
          labels = c("A","","", ""),
          rel_widths = c(0.8,0.02,1, 0.15), 
          rel_heights  = c(1,1,1,0.5), 
          axis = "rbtl")
```


#Figure 2 last updated: 2023-03-09
- Pigments heatmap
```{r}
...
- Protein heatmap - last updated: 2023-03-09 
```{r}
...
fig2_proteintopmeans <- ggplot(filter(allgroups_aggregated_means, grepl ("Bacillar|Hapto|Bacteria", class)), 
 ...
 # coord_fixed(ratio=2.5)+ #this makes it all squares 
  scale_fill_gradientn(colours = c("deepskyblue2","white","darkred"),
                       values = rescale(c(0.0000001,0.22,0.45)),
                       guide = "colorbar", limits=c(0.0000001,0.45),
                       name = "Fraction")+
   labs(x = "", y = "")+
   scale_x_discrete(limits=c("T0", "LT_noFe","LT_Fe", "HT_noFe", "HT_Fe"))+
   theme(axis.text.x=element_blank(),
...
fig2_proteinbottommeans <- ggplot(filter(allgroups_aggregated_means, grepl ("Dinophy|Ciliop|Cryptophy|Pelagophy|Chlorophy|Virus", class)), 
       aes (x = treatment2,  y = class, fill= norm_abundance))+ 
  geom_tile(color = "black", lwd = 0.7) +
  scale_fill_gradientn(colours = c("deepskyblue2","white","darkred"),
                       values = rescale(c(0.0000001,0.005,.02)),
                       guide = "colorbar", limits=c(0.0000001,0.022),
                       name = "Fraction")+
   labs(x = "", y = "")+
   scale_x_discrete(limits=c("T0", "LT_noFe","LT_Fe", "HT_noFe", "HT_Fe"))+
...
Peptide NMDS last updated: 2023-03-09 
```{r}
...
nmds = metaMDS(protein_nmds3, try = 1000, trymax = 1000, k = 2, distance = "bray") # try = 500, trymax = 500
nmds
...
data.scores$temperature_treatment <- protein_nmds2$temperature_treatment
data.scores$iron_treatment <- protein_nmds2$iron_treatment
data.scores$bioassay <- protein_nmds2$bioassay

data.scores$bioassaytempfe <- paste(data.scores$bioassay, data.scores$temperature_treatment,data.scores$iron_treatment, sep = "_")

fig2_peptide_nmds <- ggplot(data.scores, aes(x = NMDS1, y = NMDS2))+
...
Flow cytometry NMDS last updated: 2023-03-09 
```{r}
...
Pigment NMDS last updated: 2023-03-09 
```{r}
...
BA_DE2$cluster <-   ifelse (BA_DE2$cluster ==  "clust_343", "ISIP-2A", 
                   ifelse (BA_DE2$cluster ==  "clust_411", "Nitrate reductase", 
                            ...
                            ifelse (BA_DE2$cluster ==  "clust_2763", "Mn-cluster_PSBQ",
...
cluster_order <- c('ISIP-1', 'ISIP-2A', 'ISIP-3', 'Fasciclin','Cu-Zn SOD', 'Fe-Mn SOD', 'Nitrate reductase', 'Plastocyanin',  'Rhodopsin', 'Flavodoxin', 'Ferredoxin', "Cyt b6", 'Cyt f', "Chl A-B binding",'PSI RC','PSI PsaA/PsaB', "PSII PsbU", 'PSII CP47','PSII RC, D1', 'Mn-cluster', 'Mn-cluster_PSBQ' )
...
centric_ribosome <- taxon_compartment_temp("centric", 'ribo', "Centric diatom \n ribosomal protein fraction") +xlab("") + expand_limits(y = c(0,0.2)) +    xlab (expression("Temperature °C")) 
...
delete if seen any time anywhere
```{r}
 ggplot((filter(alldata, grepl("8", daycount))) , aes(
  y = (PCd_nM/(1000*PON_uM)),
  x = factor(iron_treatment, level = Fe_order),
  shape = iron_treatment))+
   scale_color_manual(name="Temperature",
                      breaks = c("LT","HT"),
  ...
  stat_summary(fun.data =  mean_se, geom = "errorbar", show.legend = F,  size = 1, width = 0.1)+
  theme_bw() + 
  ylab ("cellular cadmium concentrations") + 
  xlab (expression ("")) +
