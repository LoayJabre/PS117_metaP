...
ggplot((filter(clusters4_aggregated, grepl(clusternames, clust_annotation))), aes(x = factor(fulltreatment, level = treatment_orders),
  ...
  theme_bw() +
  #scale_color_manual(name="",  Plastocyanin

                     #breaks = c("LT","HT"),
                     #values = c( 'black','darkorange2', "black")) +
    scale_shape_manual(name="",
                     #values = c("Plastocyanin" = 1,
                               # "Haptophytes" = 16,
                                #"Dynophytes" = 4))+
...
fig2_diatomspecies <- ggplot ((filter(diatoms_aggregated, grepl("Chae|Pseu", genus))), aes(
  ...
  y = norm_abundance, 
  shape = genus)) +  
  stat_summary(fun = mean, geom = "point", show.legend = T, size = 5, alpha = 0.65)+
...
BA_DE2$cluster <-   ifelse (BA_DE2$cluster ==  "clust_343", "ISIP-2A", 
                   ifelse (BA_DE2$cluster ==  "clust_411", "Nitrate reductase", 
                            ...
                            ifelse (BA_DE2$cluster ==  "clust_6573", "Cd Carbonic Anhydrase", 
                            ...
                            ifelse (BA_DE2$cluster ==  "clust_655", "CA",
                            ...
                            ifelse (BA_DE2$cluster ==  "clust_1497", "Cyt f",BA_DE2$cluster 
                    ))))))))))))))))))))))))))))))))))))))))

cluster_order <- c('ISIP-1', 'ISIP-2A', 'ISIP-3', 'Fasciclin','Cu-Zn SOD','Fe-Mn SOD', 'Plastocyanin',  'Rhodopsin', 'Flavodoxin', 'Flavodoxin-1', 'Ferredoxin', "Cyt b6", 'Cyt f', "Chl A-B binding",'PSI RC','PSI PsaA/PsaB', "PSII PsbU", 'PSII CP47','PSII RC, D1', 'Mn-cluster', 'Urea transporter', 'Urea carboxylase', 'Nitrate reductase', 'SLC-4' )

BA_DE3 <- filter (BA_DE2, grepl (paste(cluster_order, collapse = "|"), cluster)) 

BA_DE3$BA1_ironDE <-  ifelse (BA_DE3$BA1_Fe_FDR < 0.05, "*", "")
BA_DE3$BA1_tempDE <-  ifelse (BA_DE3$BA1_temp_FDR < 0.05, "*", "")
BA_DE3$BA2_ironDE <-  ifelse (BA_DE3$BA2_Fe_FDR < 0.05, "*", "")
BA_DE3$BA2_tempDE <-  ifelse (BA_DE3$BA2_temp_FDR < 0.05, "*", "")
...
BA1_temp <- DE_slide (BA1_temp_logFC, BA1_tempDE, "BA1 Temperature Effect" ) +theme(legend.position = "none")

BA2_Fe <- DE_slide (BA2_Fe_logFC, BA2_ironDE, "BA2 Fe Effect" ) +theme(legend.position = "none") 
BA2_temp <- DE_slide (BA2_temp_logFC, BA2_tempDE, "BA2 Temperature Effect" ) +theme(legend.position = "none") 
```
...
BA1_2$cluster <-   ifelse (BA1_2$cluster ==  "clust_343", "ISIP-2A", 
                   ifelse (BA1_2$cluster ==  "clust_411", "Nitrate reductase", 
                            ...
                            ifelse (BA1_2$cluster ==  "clust_6573", "Cd Carbonic Anhydrase", 
                            ...
                            ifelse (BA1_2$cluster ==  "clust_655", "CA",
