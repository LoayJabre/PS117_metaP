This script takes the final database searching pipeline output (.cvs files), and assigns a taxonomic group and a functional annotation to each peptide using the database's complimentary annotation file. It does not assign taxonomic/functional annotation to any peptide that matches with more than one taxonomic group (I might tackle razor peptides, or peptides that match to more than one taxonomy at a later point). 
...
#Step 1 - last updated: 2023-02-27
...
lapply(filelist, function(x) {
      ...
      write.csv(paste0("cleaned_openms_output_20230203_", x), row.names = FALSE) #re-write a new 'cleaned up' .csv file. This keeps original .csv files  
        })
...
##changes the column names to include sample ID (treatment) and injection number
...
write.csv(combined_v3, "cleaned_openms_output_allsamples_20230227.csv", row.names=FALSE)
```

#Step 2 - last updated: 2023-02-27
- Adds taxonomy and functional annotation to each peptide
- Peptides that match >1 ORF and/or taxonomy  will have >1 functional and or taxnomy annotation. Next chunk deals with this
```{r}
combined_v3 <- read.csv("cleaned_openms_output_allsamples_20230227.csv")
...
combined_v4 <- combined_v3 %>%
      separate_rows(orf_id, sep = "/") %>% #some peptides match with more than one ORF; this sparates each of these matches into a row
      ...
      replace(is.na(.), "not_in_annotation") %>% #there are two types of missing annotation data here, 'NA' and empty column. NA means the peptide (ORF) is not in the annotation file all together, empty means peptide (ORF) is in annotation file, but is not clusters / doesnt't have an annotation.So I'm replacing NA's with 'not in annotation' to clarify. 
...
- Pull out and combine all the unique annotations for each peptide. For example, if a peptide matches to several ORFs of different clusters, it's 'uniq_cluster' will be a string of all those clusters. If it matches to several ORFs of the same cluster, it will just have that one cluster in its uniq_cluster annotation. 
```{r}
...
## make the strings into characters, otherwise they're a pain
...
#Step 5 - last updated: 2023-02-28
- Add the entire string of taxonomy down to the highest taxonomic resolution we can confidently ID 
- This is a bit tricky because some taxonomic assignments have numbers, spaces and other weird characters ("+", "( )") in them. Be super careful here
```{r}
...
combined_v6$final_taxon_id_adjusted <- gsub("\\+|\\(|\\)",'',combined_v6$final_taxon_id )
...
combined_v6 <- combined_v6 %>% 
                mutate (taxon_string_new =  ifelse (is.na(taxon_string_new), final_taxon_id_adjusted, 
                                                  taxon_string_new))
...
write.csv(combined_v8, "combined_v8_20230228.csv", row.names = FALSE)
...
#Step 6 - last updated: 2023-02-28
...
mcl_annotation$cluster_annotation <- paste(mcl_annotation$cluster_size, mcl_annotation$ann_type, mcl_annotation$ann_id,
...
combined_v9 <- combined_v8  %>% 
               mutate( cluster = ifelse (grepl('")', combined_v8$uniq_cluster), "ambiguous_cluster",
                                  combined_v8$uniq_cluster)) #create column with unique cluster annotations or 'ambiguous

combined_v9$cluster <- gsub(" ","", as.character (combined_v9$cluster)) #remove spaces from cluster names

## find out if all the unique clusters in the data are in the cluster annotation
## it turns out that there are several clusters e.g. "clust_480542", "clust_370505", "clust_9249" not in the cluster annotation. I manually checked these clusters, and they all have one peptide matched to them, they're measured in <3 samples total, the ORF doesn't have annotation etc. So they're low quality, and therefore I'm comfortable with them not being there. 
...
proteomicsdata3 <- proteomicsdata2[c(3:6, 1,2, 7:130)]

write.csv(proteomicsdata3, "all_ps117_taxon_function_peptides_non-normalized_injection_means_20230206.csv", row.names = FALSE)
```



#Step 7 - 
get injection average -because we injected each biological replicate twice, we need to get the average of each of those injections for DE work and other stats #20230203
```{r}
...
#after this step, I manually fixed some of the taxonomy annotations because some of the taxa names had weird formatting. So the new file has the name: 
#all_ps117_taxon_function_peptides_non-normalized_injection_means_20230204.csv
```
